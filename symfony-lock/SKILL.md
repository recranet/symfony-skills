---
name: symfony-lock
description: Prevent concurrent execution with symfony/lock - mutexes, TTLs and refresh, blocking acquire, shared stores. Use whenever a Symfony task must not run twice at once - overlapping crons, webhook dedupe, serialized access to a resource - even if the user asks for Redis SETNX or flock.
---
