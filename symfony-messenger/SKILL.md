...
description: Async and background work with symfony/messenger - messages, handlers, transports, retries, failure transport, explicit routing, flush ownership, sync vs async dispatch conventions. Use for any queue, background job, worker, event bus, or side-effect dispatch task in a Symfony project, even if the user names RabbitMQ, SQS, or Redis directly.
---
...
read `references/imports.md` in the `symfony-doctrine` skill ("Delivering
the skipped side effects"). These
