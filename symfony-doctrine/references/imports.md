...
flushes itself. Full conventions in the `symfony-messenger` skill.
...
flush itself). Follow the sync → async checklist in the
...
- **Profile before tuning** — for API imports, network latency and rate
  ...
  see the `symfony` skill's `references/debugging.md`.
- **Return `Command::SUCCESS`/`FAILURE`, never `exit()`** — see the
  `symfony-console` skill.
...
- Messenger (doctrine transport, routing): https://symfony.com/doc/7.4/messenger.html — see also the `symfony-messenger` skill
