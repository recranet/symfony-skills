...
description: Doctrine ORM in Symfony - entities, repositories, migrations, query patterns, and memory-safe batch imports. Use for any database or model work in a Symfony project - adding fields, relations, indexes, repositories, migrations, bulk updates or large imports - even if the user only mentions SQL.
---
...
For console commands that walk a large or growing dataset (imports,
backfills, re-syncs — any "loop over every row" job), read
[references/imports.md](references/imports.md) before writing code:
it covers ID-first loops, per-iteration `clear()`, DBAL side effects,
and sync-dispatched side effects.
